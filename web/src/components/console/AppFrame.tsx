// Pumpy is a handheld: full-screen on phone and framed as a focused product shot on desktop.
...
export function AppFrame({ children, bg, dimmed = false }: { children: ReactNode; bg?: string; dimmed?: boolean }) {
  ...
  return (
    <div
      ...
      style={style}
    >
      {/* Desktop surround: a quiet broadcast grid keeps attention on the device. */}
      <div
        className={cnm('pumpy-surround pointer-events-none absolute inset-0 hidden sm:block', dimmed && 'pumpy-surround-dim')}
        aria-hidden
      />
