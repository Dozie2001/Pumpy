...
// User SFX volume (0..1) applied on top of SYNTH_LEVEL; the master mute (`enabled`) still wins.
let sfxScale = 1
let synthBus: GainNode | null = null
function out(ac: AudioContext): AudioNode {
  if (!synthBus) {
    synthBus = ac.createGain()
    synthBus.gain.value = SYNTH_LEVEL * sfxScale
    synthBus.connect(ac.destination)
  }
  return synthBus
}

// Scales every synth sting by the user's SFX volume, driven from the sounds drawer via lib/audio.ts.
export function setSynthSfxVolume(v: number): void {
  sfxScale = Math.max(0, Math.min(1, v))
  if (synthBus) synthBus.gain.value = SYNTH_LEVEL * sfxScale
}

// Per-game synth beds are unlinked for now: background music comes from the mp3 player in lib/audio.ts.
// The bed code below is kept intact; flip this back to true to restore the per-game beds.
const GAME_BEDS_ENABLED = false
...
// Reward flourish: coins tumbling in, then a bright major bloom.
...
export function startLuckyBgm(): void {
  if (!GAME_BEDS_ENABLED || !enabled || luckyTimer) return
  const ac = audio()
  if (!ac) return
  ensureRunning(ac)
...
export function startBgm(): void {
  if (!GAME_BEDS_ENABLED || !enabled || bgmTimer) return
  const ac = audio()
  if (!ac) return
  ensureRunning(ac)
  const bus = ac.createGain()
...
export function startRangeBgm(): void {
  if (!GAME_BEDS_ENABLED || !enabled || rangeTimer) return
  const ac = audio()
  if (!ac) return
  ensureRunning(ac)
  const bus = ac.createGain()
...
export function startMoonshotBgm(): void {
  if (!GAME_BEDS_ENABLED || !enabled || moonTimer) return
  const ac = audio()
  if (!ac) return
  ensureRunning(ac)
  const bus = ac.createGain()
...
export function startRideBgm(): void {
  if (!GAME_BEDS_ENABLED || !enabled || rideTimer) return
  const ac = audio()
  if (!ac) return
  ensureRunning(ac)
  const bus = ac.createGain()
